...
void
handler_sigalarm (int sig)
{
  if (sig != SIGALRM)
    return;
  build_echo_request ();
  if (sendto (ping.socket_fd, ping.echo_request, ECHO_REQUEST_SIZE, 0,
              (struct sockaddr *)&ping.sendto_remote_host,
              ping.sendto_remote_host_len)
      < 0)
    FATAL_ERROR ("sendto()");
...
handler_sigint (int sig)
{
  ...
  double package_loss_percent
      = (1.0 - ((double)ping.packets_received / ping.packets_transmitted))
        * 100;
